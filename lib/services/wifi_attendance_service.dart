...
const Duration _kBackendApiBackoffDuration = Duration(minutes: 10);
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (_isBackendUnavailableResponse(response)) {
        _activateApiBackoff(
          prefs,
          'wifi-status:${response.statusCode}',
        );
      }
      final message = _extractApiMessage(response.body);
...
void _activateApiBackoff(
  SharedPreferences prefs,
  String reason,
) {
  final until = DateTime.now().add(_kBackendApiBackoffDuration);
  prefs.setInt(
    Preferences.WIFI_API_BACKOFF_UNTIL_MS,
    until.millisecondsSinceEpoch,
  );
  log('[WifiAttendance] ⛔ Backend unavailable ($reason). Pausing attendance API calls until $until');
}

bool _isApiBackoffActive(SharedPreferences prefs) {
  final untilMs = prefs.getInt(Preferences.WIFI_API_BACKOFF_UNTIL_MS) ?? 0;
  return untilMs > DateTime.now().millisecondsSinceEpoch;
}

String _remainingBackoff(SharedPreferences prefs) {
  final untilMs = prefs.getInt(Preferences.WIFI_API_BACKOFF_UNTIL_MS) ?? 0;
  if (untilMs <= DateTime.now().millisecondsSinceEpoch) {
    return '0m';
  }

  final remaining = Duration(
    milliseconds: untilMs - DateTime.now().millisecondsSinceEpoch,
  );
...
Future<String> _fetchAttendanceStatus(
  ...
  try {
    ...
    if (_isBackendUnavailableResponse(response)) {
      ...
      log('[WifiAttendance] ⚠️ Backend unavailable during attendance-status, using cached: $cached');
      return cached;
    }
...
Future<void> wifiAttendanceServiceMain(ServiceInstance service) async {
  ...
  Future<void> checkAndSyncWifiStatus() async {
    ...
    try {
      ...
      if (_isApiBackoffActive(prefs)) {
        log('[WifiAttendance] ⏸️ Skipping poll cycle due to backend backoff (${_remainingBackoff(prefs)} remaining)');
        return;
      }
      ...
      if (status == 'connected' || bssidForApi.isNotEmpty) {
        ...
        log('[WifiAttendance] skipping wifi-status post because no office BSSID is cached for disconnected state');
      }
